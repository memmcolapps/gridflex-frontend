...
export default function AssignMeterPage() {
  ...
  >(null);
  const [editCustomer, setEditCustomer] = useState<MeterInventoryItem | null>(
    null,
  );
  const [migrateCustomer, setMigrateCustomer] =
  ...
  const [customerToDetach, setCustomerToDetach] =
    useState<MeterInventoryItem | null>(null);
  const [isDetachConfirmModalOpen, setIsDetachConfirmModalOpen] =
    useState(false);
  ...
  const currentPageData = meterData;
  ...
  const handleMigrateMeter = (
    customer: MeterInventoryItem | VirtualMeterData,
  ) => {
    if ("meterManufacturer" in customer) {
      setMigrateCustomer(customer);
      setMigrateToCategory(
        customer.category === "Postpaid" ? "Prepaid" : "Postpaid",
      );
  ...
  const handleSortChange = (
    key: keyof MeterInventoryItem | keyof VirtualMeterData,
    direction: "asc" | "desc",
  ) => {
        ...
        <div className="mb-4 flex w-full items-center gap-2 md:w-auto">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="center" className="w-48">
              ...
              <DropdownMenuItem
                onClick={() => handleSortChange("customerId", "asc")}
                className="cursor-pointer text-sm hover:bg-gray-100"
              >
                Customer ID: A to Z
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => handleSortChange("customerId", "desc")}
                className="cursor-pointer text-sm hover:bg-gray-100"
              >
                Customer ID: Z to A
              </DropdownMenuItem>
            ...
            <>
              {currentPageData.map((meter, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="px-4 py-3 text-center">
                    {meter.creditPaymentPlan ?? "-"}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-center">
                    <span
                      className={cn(
                        "inline-block text-sm font-medium",
                        getStatusStyle(meter.meterStage ?? meter.status),
                      )}
                    >
                  ...
                  <TableCell>
                    {(meter.meterStage ?? meter.status)?.toLowerCase() ===
                    "pending-approval" ? (
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
