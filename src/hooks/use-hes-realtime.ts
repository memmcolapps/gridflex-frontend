...
export const hesQueryKeys = {
  all: ["hes"] as const,
  realtimeData: (meterNo?: string) =>
    [...hesQueryKeys.all, "realtime-data", meterNo] as const,
  connectionStatus: (meterNo?: string) =>
    [...hesQueryKeys.all, "connection-status", meterNo] as const,
...
export function useSSEManagement() {
  ...
  const [connectionStatus, setConnectionStatus] =
    useState<MeterConnectionStatus>({});
...
export function useRealtimeStream() {
  return useMutation({
    mutationFn: async ({
      ...
      const result = await triggerRealtimeStream(payload, callbacks);
      if (!result.success) {
