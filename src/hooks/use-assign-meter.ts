...
const mapToFrontendMeter = (
  apiItem: MeterAPIItem,
  isVirtual: boolean,
  ...
  if (isVirtual) {
    return {
      ...
      streetName:
        apiItem.meterAssignLocation?.streetName ?? apiItem.customer?.streetName,
      houseNo:
        apiItem.meterAssignLocation?.houseNo ?? apiItem.customer?.houseNo,
      image: null,
  ...
  return {
    ...
    accountNumber: apiItem.accountNumber,
    cin: apiItem.cin,
    tariff: apiItem.tariffInfo?.name ?? apiItem.tariff,
    dss: apiItem.dssInfo?.name ?? apiItem.dss,
    feederLine: apiItem.feederInfo?.name ?? "",
    feederInfo: apiItem.feederInfo,
    dssInfo: apiItem.dssInfo,
    tariffInfo: apiItem.tariffInfo,
...
export const useMeters = ({
  ...
  return useQuery<
    ...
    Error,
    {
      actualMeters: MeterInventoryItem[];
      virtualMeters: VirtualMeterData[];
      totalData: number;
    }
  >({
    queryKey: [
...
export const useAssignMeter = () => {
  ...
  >({
    ...
    onSuccess: (data) => {
      ...
      // Note: response code 131 (CIN already exists) is handled in the component
    },
    onError: (error: Error) => {
...
export const useContinueAssignMeter = () => {
  const queryClient = useQueryClient();
  return useMutation<
    {
      responsecode: string;
...
export const useSaveVirtualMeter = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, meter }: { id: string; meter: VirtualMeterPayload }) =>
      saveVirtualMeter(id, meter),
    onSuccess: () => {
