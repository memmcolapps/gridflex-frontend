...
export const useCreateLiabilityCause = () => {
  ...
  return useMutation({
    mutationFn: (payload: LiabilityCausePayload) =>
      createLiabilityCause(payload),
    onSuccess: (response) => {
      if (response.success) {
...
export const useUpdateLiabilityCause = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: UpdatedLiabilityCausePayload) =>
      updateLiabilityCause(payload),
    onSuccess: (response) => {
      if (response.success) {
...
export const useChangeLiabilityCauseStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ id, status }: { id: string; status: boolean }) =>
      changeLiabilityCauseStatus(id, status),
    onSuccess: (response) => {
      if (response.success) {
...
export const useCreatePercentageRange = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: PercentageRangePayload) =>
      createPercentageRange(payload),
    onSuccess: () => {
...
export const useUpdatePercentageRange = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: UpdatedPercentageRangePayload) =>
      updatePercentageRange(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["percentageRange"] });
    },
    onError: (error) => {
...
export const useChangePercentageRangeStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ id, status }: { id: string; status: boolean }) =>
      changePercentageRangeStatus(id, status),
    onSuccess: () => {
...
export const useBulkApproveLiabilityCauses = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (liabilityCauses: { name: string }[]) =>
      bulkApproveLiabilityCauses(liabilityCauses),
    onSuccess: (response) => {
      if (response.success) {
        toast.success(
          `${response.data.successCount} of ${response.data.totalRecords} liability causes approved successfully!`,
        );
        queryClient.invalidateQueries({ queryKey: ["liability"] });
      } else {
        toast.error(response.error);
...
export const useBulkApprovePercentageRanges = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (percentageRanges: { code: string }[]) =>
      bulkApprovePercentageRanges(percentageRanges),
    onSuccess: (response) => {
      if (response.success) {
        toast.success(
          `${response.data.successCount} of ${response.data.totalRecords} percentage ranges approved successfully!`,
        );
        queryClient.invalidateQueries({ queryKey: ["percentageRange"] });
      } else {
        toast.error(response.error);
