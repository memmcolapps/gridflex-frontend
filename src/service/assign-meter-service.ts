...
export async function getMeters({
  ...
  try {
    ...
    const response = await axiosInstance.get(`${API_URL}/meter/service/all`, {
      params,
      headers: {
    ...
    if (response.data.responsecode !== "000") {
      throw new Error(
        response.data.responsedesc ?? "Failed to fetch meter data.",
      );
    }
...
export async function saveMeter(
  ...
  try {
    ...
    const response = await axiosInstance.post(`${API_URL}/api/meters`, meter, {
      headers: {
        "Content-Type": "application/json",
        custom: CUSTOM_HEADER,
        Authorization: `Bearer ${token}`,
...
export async function changeMeterState({
  meterId,
  status,
  reason,
}: ChangeMeterStateParams): Promise<{
  responsecode: string;
  responsedesc: string;
}> {
  try {
    ...
    const response = await axiosInstance.patch(
      `${API_URL}/meter/service/change-state`,
      null,
      {
        params,
        headers: {
    ...
    if (response.data.responsecode !== "000") {
      throw new Error(
        response.data.responsedesc ??
          `Failed to ${status ? "activate" : "deactivate"} meter.`,
      );
    }
    return response.data;
  } catch (error) {
...
export async function migrateMeter(
  data: MigrateMeterPayload,
): Promise<{ responsecode: string; responsedesc: string }> {
  try {
    const token = localStorage.getItem("auth_token");
    if (!token) throw new Error("Authentication token not found.");
...
export async function saveVirtualMeter(
  id: string,
  meter: VirtualMeterPayload,
): Promise<{ responsecode: string; responsedesc: string }> {
  try {
    const token = localStorage.getItem("auth_token");
    if (!token) {
    ...
    if (response.data.responsecode !== "000") {
      throw new Error(
        response.data.responsedesc ?? "Failed to save virtual meter.",
      );
    }
...
export async function deactivatePhysicalMeter(
  id: string,
): Promise<{ responsecode: string; responsedesc: string }> {
  try {
    const token = localStorage.getItem("auth_token");
    ...
    const response = await axiosInstance.post(
      `${API_URL}/api/meters/${id}/deactivate`,
      {},
      {
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.data.responsecode !== "000") {
      throw new Error(
        response.data.responsedesc ?? "Failed to deactivate physical meter.",
      );
    }
...
export async function createVirtualMeter(
  meter: VirtualMeterPayload,
): Promise<{ responsecode: string; responsedesc: string }> {
  try {
    const token = localStorage.getItem("auth_token");
    ...
    if (response.data.responsecode !== "000") {
      throw new Error(
        response.data.responsedesc ?? "Failed to create virtual meter.",
      );
    }
