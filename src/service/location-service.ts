...
export const fetchNigerianCitiesByState = async (
    ...
    try {
        ...
        const selectedState = response.data.find(
            (item: GeoJSONState) => item.state.toLowerCase().replace(/\s+/g, '-') === state
        );
        ...
        console.error(`Error fetching cities for state ${state} from GeoJSON:`, error);
        
        throw new Error(`Failed to fetch cities for state ${state} from GeoJSON data. Please check the URL or your network connection.`);
    }
};
