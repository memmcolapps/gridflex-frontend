...
export default function EnergyImportTable({
  ...
  const [isEditReadingsDialogOpen, setIsEditReadingsDialogOpen] =
    useState(false);
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    if (key === "feederName" || key === "assetId") {
      const aValue = a[key as keyof EnergyImportData];
      const bValue = b[key as keyof EnergyImportData];
  ...
  // Use API pagination if provided, otherwise use local pagination
  ...
  return (
    <Card className="border-none bg-transparent p-4">
      ...
      <PaginationControls
        ...
        totalItems={onPageChange ? totalCount : sortedData.length}
        pageSize={pageSize}
      ...
      {selectedItem && (
        <Dialog
          open={isEditReadingsDialogOpen}
          onOpenChange={setIsEditReadingsDialogOpen}
        >
