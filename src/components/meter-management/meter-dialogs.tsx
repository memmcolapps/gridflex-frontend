...
interface DeactivateDialogProps {
    isOpen: boolean;
    onClose: () => void;
    onDeactivate: (reason?: string) => void; // Modified to accept optional reason
    meterNumber: string;
    action: "deactivate" | "activate"; // Added action prop
}

export function DeactivateDialog({ isOpen, onClose, onDeactivate, action }: DeactivateDialogProps) {
    const [reason, setReason] = useState<string>("");
    const [isFinalConfirmOpen, setIsFinalConfirmOpen] = useState(false);

    const handleProceed = () => {
        if (action === "deactivate" && !reason.trim()) return; // Require reason for deactivation
        setIsFinalConfirmOpen(true);
        onClose();
    };

    const handleFinalConfirm = () => {
        onDeactivate(action === "deactivate" ? reason : undefined); // Pass reason only for deactivation
        setReason("");
            ...
            <Dialog open={isOpen} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-[350px] h-fit bg-white rounded-lg p-6">
                    ...
                    <DialogFooter className="flex justify-end gap-3 mt-4">
                        ...
                        <Button
                            className={
                                action === "deactivate" && !reason.trim()
                                    ? "bg-red-200 text-white cursor-not-allowed"
                                    : "bg-[#F50202] text-white hover:bg-red-700 cursor-pointer"
                            }
                            onClick={handleProceed}
                            disabled={action === "deactivate" && !reason.trim()}
                        >
                ...
                <DialogContent className="sm:max-w-[350px] h-fit bg-white rounded-lg p-6">
                    ...
                    <div className="">
                        <p className="text-sm text-gray-700">
                            Are you sure you want to {action === "deactivate" ? "deactivate" : "activate"} the meter?
                        </p>
                    </div>
