...
import {
  useGenerateCreditToken,
  usePrintToken,
  useCalculateCreditToken,
  useGenerateKCTToken,
  useGenerateClearTamperToken,
...
export default function TokenFormDialog({ tokenType }: TokenFormDialogProps) {
  ...
  const handleProceed = async () => {
    if (
      ...
      try {
        const result =
          await generateClearTamperTokenMutation.mutateAsync(payload);
        setGeneratedTokenData(result);
        setShowTokenDialog(true);
        setIsFormDialogOpen(false);
      } catch (error) {
      ...
      try {
        const result =
          await generateClearCreditTokenMutation.mutateAsync(payload);
        setGeneratedTokenData(result);
        setShowTokenDialog(true);
        setIsFormDialogOpen(false);
      } catch (error) {
      ...
      try {
        const result =
          await generateCompensationTokenMutation.mutateAsync(payload);
        setGeneratedTokenData(result);
        setShowTokenDialog(true);
        setIsFormDialogOpen(false);
      } catch (error) {
                    ...
                    <body>
                        <div class="receipt">
                            ...
                            <div class="info-row">
                                ...
                                <span class="value">${generatedTokenData?.createdAt ? new Date(generatedTokenData.createdAt).toLocaleString("en-US", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "N/A"}</span>
                            </div>
                            <div class="info-row">
                            ...
                            <div class="amount-section">
                                <div class="info-row">
                                    ...
                                    <span class="value">₦${generatedTokenData?.lastAmountVended?.toLocaleString() ?? "0"}</span>
                                </div>
                                <div class="info-row">
            ...
            {!showReceipt && !showTokenDialog && (
              <>
                <Button
                  ...
                  onClick={() => {
                    if (tokenType === "creditToken" && isKctStepOpen) {
                      setIsKctStepOpen(false);
                      return;
                    }
                    setIsFormDialogOpen(false);
        ...
        <DialogContent className="h-fit w-full bg-white">
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <p>
                {generatedTokenData?.createdAt
                  ? new Date(generatedTokenData.createdAt).toLocaleString()
                  : "N/A"}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            {tokenType === "compensation" && (
              <div className="grid grid-cols-2 gap-4">
                <p>
                  <strong>Compensation Token:</strong>
                </p>
                <p>{generatedTokenData?.token}</p>
              </div>
            )}
