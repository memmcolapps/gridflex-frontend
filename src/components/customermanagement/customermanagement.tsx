      ...
      bulkUploadMutation.mutate(data, {
        onSuccess: (response: unknown) => {
          ...
          if (res.responsedata.successCount > 0) {
            toast.success(
              `${res.responsedata.successCount} of ${res.responsedata.totalRecords} customers uploaded successfully!`,
            );
          }
        },
        onError: (error) => {
    ...
    <div className="flex h-full flex-col overflow-hidden text-black">
      <div className="flex-grow">
          ...
          {canEdit && (
            <div className="flex gap-4">
              <Button
                ...
                variant="outline"
                size={"lg"}
                className="cursor-pointer border-[#161CCA] py-4 text-[#161CCA]"
              >
        ...
        <BlockCustomerDialog
          ...
          onConfirmUnblock={confirmUnblockCustomer}
        />
                    ...
                    {uploadResult.successCount > 0 && (
                      <div className="rounded-lg border border-green-200 bg-green-50 p-3">
                        ...
                        <p className="mt-1 text-sm text-green-700">
                          {uploadResult.successCount} customer
                          {uploadResult.successCount !== 1 ? "s" : ""} uploaded
                          successfully.
                        </p>
                      </div>
                    )}
