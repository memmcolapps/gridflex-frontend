...
const PercentageRangeTable = ({
  ...
  const [confirmAction, setConfirmAction] = useState<
    "approve" | "reject" | null
  >(null);
  const [selectedItem, setSelectedItem] = useState<PercentageRange | null>(
    null,
  );
  ...
  if (isLoading)
    return (
      <div className="flex min-h-96 items-center justify-center">
        <LoadingAnimation
          variant="spinner"
          message="Loading percentage ranges..."
          size="lg"
        />
      </div>
    );
            ...
            <TableHead className="w-[100px] px-4 py-3">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  checked={
                    selectedPercentageRangeCodes.length ===
                      dataToDisplay.length && dataToDisplay.length > 0
                  }
                  onCheckedChange={(checked) => {
                    if (checked) {
                      setSelectedPercentageRangeCodes(
                        filteredPercentageRanges.map((item) => item.code),
                      );
                    } else {
                      setSelectedPercentageRangeCodes([]);
                    }
