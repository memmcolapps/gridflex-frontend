...
const MeterTable = ({
  ...
  const toggleSelection = (meterNumber: string) => {
    setSelectedMeterNumbers((prev) =>
      prev.includes(meterNumber)
        ? prev.filter((item) => item !== meterNumber)
        : [...prev, meterNumber],
    );
  };
            ...
            <TableHead className="w-[100px] px-4 py-3">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  checked={
                    selectedMeterNumbers.length === paginatedData.length &&
                    paginatedData.length > 0
                  }
                  onCheckedChange={(checked) => {
                    if (checked) {
                      setSelectedMeterNumbers(
                        paginatedData.map((item) => item.meterNumber),
                      );
                    } else {
                      setSelectedMeterNumbers([]);
                    }
          ...
          {paginatedData.length === 0 ? (
            ...
            paginatedData.map((item, index) => (
              ...
              >
                ...
                <TableCell className="px-4 py-3 text-sm text-gray-900">
                  {item.oldMeterInfo?.manufacturer?.name || "-"}
                </TableCell>
                <TableCell className="px-4 py-3 text-sm text-gray-900">
                  {item.meterClass}
                </TableCell>
