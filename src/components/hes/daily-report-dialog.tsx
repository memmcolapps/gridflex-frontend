...
export function DailyReportDialog({ open, onOpenChange, activeTab }: DailyReportDialogProps) {
                    ...
                    <>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid grid-cols-1 items-center gap-2">
                                ...
                                <Popover open={isMeterDropdownOpen} onOpenChange={setIsMeterDropdownOpen}>
                                    <PopoverTrigger asChild>
                                        <Button
                                            variant="outline"
                                            role="combobox"
                                                    ...
                                                    {filteredMeters.map((meter) => (
                                                        <CommandItem
                                                            ...
                                                            onSelect={(currentValue) => {
                                                                const isSelected = selectedMeters.includes(currentValue);
                                                                if (isSelected) {
                                                                    setSelectedMeters(selectedMeters.filter(m => m !== currentValue));
                                                                } else {
                                                                    setSelectedMeters([...selectedMeters, currentValue]);
                                                                }
                                                            }}
                                                        >
                                                            <Check
                                                                className={cn(
                                                                    "mr-2 h-4 w-4",
                                                                    selectedMeters.includes(meter.meterNumber) ? "opacity-100" : "opacity-0"
                                                                )}
                                                            />
                                                            {meter.meterNumber}
                                                        </CommandItem>
