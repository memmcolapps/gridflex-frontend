...
import { format, setHours, setMinutes } from "date-fns"; // Added setMinutes import
...
const eventOptionsByType: Record<string, string[]> = {
  ...
  "Token Event Log": ["Select All", "Recharge Token Event", "Token Event"],
};
...
const [selectedEventType, setSelectedEventType] = useState("Relay Control Log");
const [selectedEvents, setSelectedEvents] = useState("Select All");
...
useEffect(() => {
  const options = eventOptionsByType[selectedEventType] || ["Select All"];
  setAvailableEventOptions(options);
  // Reset selected events to "Select All" when event type changes
  setSelectedEvents("Select All");
}, [selectedEventType]);

const handleEventTypeChange = (eventType: string) => {
  setSelectedEventType(eventType);
  // The useEffect will handle updating the available options and resetting selected events
};
            ...
            >
              {availableEventOptions.map((event) => (
                <DropdownMenuItem
                  ...
                  className={selectedEvents === event ? "bg-gray-100" : ""}
                >
                  {event}
                </DropdownMenuItem>
              ))}
